...
import io.domisum.lib.youtubeapilib.action.upload.VideoThumbnailUploader;
		...
		implements VideoThumbnailUploader
{
	...
	// UPLOAD
	@Override
	public void uploadThumbnail(String videoId, BufferedImage thumbnail)
			throws IOException
	{
		logger.info("Preparing to upload thumbnail for video '{}'", videoId);
		
		// remove alpha channel because we upload as jpg
	...
	// REQUEST
	private Set createApiRequest(String videoId, InputStreamContent imageContent)
			throws IOException
	{
		var thumbnailSet = authorizedYouTubeApiClient.getYouTubeApiClient().thumbnails().set(videoId, imageContent);
